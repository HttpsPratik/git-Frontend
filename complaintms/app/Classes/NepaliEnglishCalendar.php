...
class NepaliEnglishCalendar
{
    ...
    public function eng_to_nep($yy, $mm, $dd)
    {
        ...
        else
        {
            ...
            $numDay = $day;
            $this->_nep_date['year']    = $this->eng_to_nep_num($y);
            $this->_nep_date['month']   = $this->eng_to_nep_num($m);
            $this->_nep_date['date']    = $this->eng_to_nep_num($total_nDays);
    ...
    public function eng_to_nep_num($number)
    {
        $eng_number = array(
            "0",
            "1",
    ...
    public function nep_to_eng_num($number)
    {
        $eng_number = array(
            "0",
            "1",
     ...
     * @return string
     */
    public function engToNepali($yy, $mm, $dd)
    {
        // Check for date range
        $chk = $this->_is_in_range_eng($yy, $mm, $dd);
        if($chk !== TRUE)
    ...
    /**
     * Convert english date time to nepali date time in english
     *
     * @param \DateTime
     * @return string
     */
    public function getNepaliDateTime($datetime)
    {
        $date = explode(' ', $datetime)[0];
        $time = explode(' ', $datetime)[1];
        $date = explode('-', $date);
        return $this->engToNepali($date[0], $date[1], $date[2]).' '.$time;
    }
     ...
     * @return string
     */
    public function getNepaliDateTimeA($datetime)
    {
    ...
    {
        $date = explode('-', $date);
        return $this->engToNepali($date[0], $date[1], $date[2]);
    }
}
